...
const checkServiceOwnership = async (userId) => {
  ...
  if (!service || !service.userId.equals(userId)) {
    throw new ApiError("This service does not belong to you for authorization", 403);
  }
};

// Check if notification exists in user notifications
const findNotification = (user, notificationId) => {
  return user.notifications.find(notification => notification._id.equals(notificationId));
};
...
exports.seenNotification = catchError(
  asyncHandler(async (req, res, next) => {
    ...
    res.status(200).json({ message: "Notification marked as seen", updatedUser });
  })
);
